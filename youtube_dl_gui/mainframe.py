...
class MainFrame(wx.Frame):
    ...
    def __init__(self, opt_manager, log_manager, parent=None):
        super(MainFrame,
              self).__init__(parent,
                             wx.ID_ANY,
                             __appname__,
                             size=opt_manager.options["main_win_size"])
        ...
        self._videoformat_combobox = CustomComboBox(self._panel,
        ...
        self.Bind(wx.EVT_LIST_ITEM_SELECTED, self._update_pause_button,
                  self._status_list)
        self.Bind(wx.EVT_LIST_ITEM_DESELECTED, self._update_pause_button,
                  self._status_list)
        self.Bind(wx.EVT_CLOSE, self._on_close)
        self.Bind(wx.EVT_TIMER, self._on_timer, self._app_timer)

        self._videoformat_combobox.Bind(wx.EVT_COMBOBOX,
    ...
    def _update_videoformat_combobox(self):
        ...
        self._videoformat_combobox.add_items(list(DEFAULT_FORMATS.values()),
                                             False)
    ...
    def _update_videoformat(self, event):
        self.opt_manager.options[
            "selected_format"] = selected_format = FORMATS[
    ...
    def _on_pause(self, event):
        ...
        if selected_rows:
            ...
            for selected_row in selected_rows:
                ...
                self._status_list._update_from_item(selected_row,
                                                    download_item)
    ...
    def _on_add(self, event):
        ...
        if not urls:
            self._create_popup(self.PROVIDE_URL_MSG, self.WARNING_LABEL,
                               wx.OK | wx.ICON_EXCLAMATION)
        else:
    ...
    def _create_bitmap_button(self, icon, size=(-1, -1), handler=None):
        button = wx.BitmapButton(self._panel,
                                 bitmap=icon,
                                 size=size,
                                 style=wx.NO_BORDER)
    ...
    def _after_download(self):
        ...
        if self.opt_manager.options["shutdown"]:
            dlg = ShutdownDialog(self, 60, _("Shutting down in {0} second(s)"),
                                 _("Shutdown"))
            ...
            if self.opt_manager.options["show_completion_popup"]:
                self._create_popup(self.DL_COMPLETED_MSG, self.INFO_LABEL,
                                   wx.OK | wx.ICON_INFORMATION)
    ...
    def _on_close(self, event):
        ...
        if (self.download_manager is not None
                and self.opt_manager.options["confirm_exit"]):
            dlg = wx.MessageDialog(
                self,
    ...
    def close(self):
        ...
        self.opt_manager.options[
            "opts_win_size"] = self._options_frame.GetSize()

        self.opt_manager.options[
            "save_path_dirs"] = self._path_combobox.GetStrings()
...
class ListCtrl(wx.ListCtrl, ListCtrlAutoWidthMixin):
    ...
    def get_all_selected(self):
        return [
            index for index in range(self._list_index)
    ...
    def _set_columns(self):
        ...
        for column_item in sorted(self.columns.values()):
            self.InsertColumn(column_item[0],
                              column_item[1],
...
class ButtonsChoiceDialog(wx.Dialog):
    ...
    def __init__(self, parent, choices, message, *args, **kwargs):
        super(ButtonsChoiceDialog, self).__init__(parent,
                                                  wx.ID_ANY,
                                                  *args,
                                                  style=self.STYLE,
                                                  **kwargs)
        ...
        info_bmp = wx.ArtProvider.GetBitmap(wx.ART_INFORMATION,
                                            wx.ART_MESSAGE_BOX)
...
class ShutdownDialog(wx.Dialog):
    ...
    def __init__(self, parent, timeout, message, *args, **kwargs):
        ...
        self.msg_text = msg_text = wx.StaticText(panel, wx.ID_ANY,
                                                 self._get_message())
